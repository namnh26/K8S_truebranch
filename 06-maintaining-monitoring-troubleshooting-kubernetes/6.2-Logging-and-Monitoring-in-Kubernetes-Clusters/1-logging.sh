...
#But, what if your control plane is down? Go to crictl --runtime-endpoint unix:///run/containerd/containerd.sock or to the file system.
#kubectl logs will send the request to the local node's kubelet to read the logs from disk
#Since we're on the Control Plane Node/control plane node already we can use crictl --runtime-endpoint unix:///run/containerd/containerd.sock for that.
...
#But, what if crictl --runtime-endpoint unix:///run/containerd/containerd.sock is not available?
...
#These are json formmatted which is the crictl --runtime-endpoint unix:///run/containerd/containerd.sock logging driver default

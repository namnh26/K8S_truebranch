...
#Our Pod Network was defined in the first course and we chose 10.244.0.0/16
...
#For the Pod on c1-node, let's find out how traffic gets from c1-cp1 to c1-node1 to get to that Pod.

#Look at the annotations, specifically the annotation projectcalico.org/IPv4IPIPTunnelAddr: 10.244.13.192...your IP may vary
#Check out the Addresses: InternalIP, that's the real IP of the Node.
# Pod IPs are allocated from the network Pod Network which is configurable, it's controlling the IP allocation.
...
#Via routes on the node, to get to c1-node1 traffic goes into tunl0/10.244.222.192...your IP may vary
...
#The local tunl0 is 10.244.13.192, packets destined for Pods running on c1-cp1 will be routed to this interface and get encapsulated
#Then send to the destination node for de-encapsulation.
ip addr


#Log into c1-node1 and look at the interfaces, there's tunl0 10.244.222.192...this is this node's tunnel interface
ssh aen@c1-node1


#This tunl0 is the destination interface, on this Node its 10.244.222.192, which we saw on the route listing on c1-cp1
ip addr
...
#UPDATE: New SSH Access Method see link above for details...the single line of code on 102 will get you terminal access to a node
